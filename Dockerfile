...
RUN echo 'Building in release mode.' ; \
    cargo build --release ; \
    mv /infra-evm-parachain/target/release/infra-evm /infra-evm-parachain/target/; 
...
COPY --from=builder /infra-evm-parachain/target/infra-evm .
